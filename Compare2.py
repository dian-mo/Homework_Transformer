...
class TrainingVisualizer:
    ...
    def plot_training_curves(self, save_path='training_curves_ablation_no_attention.png'):
        """绘制训练曲线"""
        ...
        ax1.set_title('消融实验-训练和测试损失曲线(无注意力机制)', fontproperties=font, fontsize=14)
        ...
        ax2.set_title('消融实验-测试困惑度曲线(无注意力机制)', fontproperties=font, fontsize=14)
    ...
    def plot_attention_heatmap(self, attention_weights, src_tokens, tgt_tokens,
                               save_path='attention_heatmap_ablation_no_attention.png'):
        """绘制注意力热力图"""
        ...
        plt.title('消融实验-注意力机制热力图(无注意力机制)', fontproperties=font, fontsize=16)
    ...
    def plot_results_comparison(self, test_results, save_path='results_comparison_ablation_no_attention.png'):
        """绘制结果对比图"""
        ...
        ax1.set_title('消融实验-序列长度对比(无注意力机制)', fontproperties=font, fontsize=14)
        ax1.set_xticks(x)
        ...
        ax2.set_title('消融实验-词汇重叠率(无注意力机制)', fontproperties=font, fontsize=14)
        ax2.set_ylim(0, 1)
    ...
    def generate_word_cloud(self, texts, title, save_path='wordcloud_ablation_no_attention.png'):
        """生成词云图"""
        if not texts:
            return
...
class SimpleLinearLayer(nn.Module):
    ...
    def forward(self, x, mask=None):
        # 简单的线性变换，忽略mask参数以保持接口一致
        return self.linear(x), None  # 返回None作为注意力权重占位符
...
class EncoderLayerNoAttention(nn.Module):
    ...
    def __init__(self, d_model, d_ff, dropout=0.1):
        super().__init__()
        # 使用简单的线性层替换自注意力机制
        self.linear_layer = SimpleLinearLayer(d_model, dropout)
    ...
    def forward(self, x, mask=None):
        # 使用线性层替换自注意力
        x = self.sublayer1(x, lambda x: self.linear_layer(x, mask)[0])
        x = self.sublayer2(x, self.ffn)
        return x


class DecoderLayerNoAttention(nn.Module):
    """移除注意力机制的解码器层 - 消融实验"""

    def __init__(self, d_model, d_ff, dropout=0.1):
        super().__init__()
        # 使用简单的线性层替换所有注意力机制
        self.masked_linear = SimpleLinearLayer(d_model, dropout)
        self.cross_linear = SimpleLinearLayer(d_model, dropout)
        self.ffn = PositionWiseFFN(d_model, d_ff, dropout)
        self.sublayer1 = SublayerConnection(d_model, dropout)
        self.sublayer2 = SublayerConnection(d_model, dropout)
    ...
    def forward(self, x, enc_output, tgt_mask=None, src_mask=None):
        # 使用线性层替换所有注意力机制
        x = self.sublayer1(x, lambda x: self.masked_linear(x, tgt_mask)[0])
        x = self.sublayer2(x, lambda x: self.cross_linear(x, src_mask)[0])
...
class TransformerNoAttention(nn.Module):
    """移除注意力机制的Transformer模型 - 消融实验版本"""

    def __init__(self, src_vocab_size, tgt_vocab_size, d_model=256, num_layers=4, d_ff=1024, max_len=50,
                 dropout=0.1):
        super().__init__()
        ...
        # 使用无注意力机制的编码器和解码器
        self.encoders = nn.ModuleList([EncoderLayerNoAttention(d_model, d_ff, dropout) for _ in range(num_layers)])
        self.decoders = nn.ModuleList([DecoderLayerNoAttention(d_model, d_ff, dropout) for _ in range(num_layers)])
...
if __name__ == "__main__":
    ...
    max_len = 60
    d_model = 128
    num_layers = 2  # 注意：这里移除了num_heads参数，因为不再需要
    d_ff = 512
    dropout = 0.2
    lr = 1e-4
    epochs = 5
    save_path = 'ablated_model_no_attention.pth'  # 修改保存路径以区分消融实验

    # 初始化可视化器
    visualizer = TrainingVisualizer()

    print("=== 消融实验：移除多头自注意力机制 ===")
    print("加载数据...")
    ...
    print(f"消融实验模型参数数量: {sum(p.numel() for p in model.parameters()):,}")
    ...
    for epoch in range(epochs):
        ...
        if test_ppl < best_ppl:
            ...
            print(f"✅ 最佳消融模型已保存（PPL: {best_ppl:.2f}）")
